...
float x_Position = -10.0;				/*the x position that will be updated for the animation, in each frame. The smaller this value is
...
void display() {						/*In animation, a SINGLE call to the display function is equal to a single frame*/
	...
	glVertex2f(x_Position, 1.0);		//Top left corner
	glVertex2f(x_Position, -1.0);		//Bottom left corner
	glVertex2f(x_Position+2.0,-1.0);	//Bottom right corner
	glVertex2f(x_Position+2.0, 1.0);	//Top right corner
...
void init() {
	...
	glClearColor(0.0,0.0,0.0,1.0);		/*1st argument: intensity of red, 2nd argument: intensity of green, 3rd argument: intensity of blue
...
void timer(int){
	...
	switch (state) {
		case 1:							//Case state = 1 means: Moving to the right side
			if (x_Position < 8.0) {		//Updates the x_Position of our square, so it 'moves' it on the x-axis. If it's value is < 8
				x_Position += 0.12;		//it keeps updating it, if not, stops. 8 is chosen because the square is 2 units, and our ortho
		...
		case -1:						//Case state = -1 means: Moving to the left side
			if (x_Position > -10.0) {	//if x_Position > -10, meaning it it hasn't reached the left side of the window, it will continue	
				x_Position -= 0.12;		//to move left, until it reaches it.
			}
			else {
